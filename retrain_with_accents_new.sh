...
# finetune_cv_train_nz_indian4000
# finetune_cv_train_nz_ind+aus+us+eng4000

# for newName in finetune_cv_train_nz_ind+aus+us+eng4000 finetune_cv_train_nz_indian4000; do
newName=finetune_cv_train_nz_8k_2000_
# newName=finetune_cv_train_nz_ind+aus+us+eng4000
dir=exp/nnet3_msr/tdnn_d_${newName}sp

#for egs
num_utts_subset=300 #default is 512
samples_per_iter=50000 #default is 400000



mkdir -p $dir
initialDir=exp/alreadyTrainedModelsOnSwbd/tdnn_d_sp
model=$initialDir/final.mdl



train_stage=3
train_set=cv_train_nz_8k_2000
nj=30
...
#get alignments
ali_dir=exp/tri4_swbd_${train_set}_ali
if [ $alignments -eq 1 ]; then

	steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/${train_set} data/lang exp/alreadyTrainedModelsOnSwbd/tri4 ${ali_dir}
    echo "============================================="
...
if [ $ivector -eq 1 ]; then
	  ...
	  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
	    data/${train_set}_hires  exp/alreadyTrainedModelsOnSwbd/nnet_online/extractor ${online_ivector_dir} || exit 1;
	    echo "============================================="
...
graph_dir=exp/alreadyTrainedModelsOnSwbd/tri4/graph_sw1_tg
if [ $decode -eq 1 ]; then
	...
	# decodeSets="cv_dev_nz_8k test_vctk_3accs"
	decodeSets="test_vctk_3accs"
	for decode_set in $decodeSets ; do

		if [ $decode_set == "test_vctk_3accs" ]; then
			nj=12
		else 
			nj=25
		fi
