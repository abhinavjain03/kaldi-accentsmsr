...
mfccdir=exp/mfcc
if [ $mfcc -eq 1 ]; then
	for x in cv_train_nz cv_dev_nz; do
...
if [ $tdnn -eq 1 ]; then
  ...
  if [ $basic -eq 1 ]; then
    local/nnet3/run_tdnn_cv_train_everything.sh --affix cv_train_everything --stage 11 --train-stage 158
  fi
...
if [ $wer -eq 1 ]; then
  ...
  for x in exp/*/*cv_train_everything*/decode*; do [ -d $x ] && grep WER $x/wer_* | utils/best_wer.sh; done
  #for x in exp/*/*/*/decode*; do [ -d $x ] && grep WER $x/wer_* | utils/best_wer.sh; done
fi
