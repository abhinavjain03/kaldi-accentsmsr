...
dir=${dir}$suffix
train_set=cv_train_nz_8k$suffix 
ali_dir=exp/tri4_cv_train_nz_8k_ali$suffix
...
if [ $stage -le 11 ]; then
  ...
  ivectordev=0
  decodedev=1
  devsets="test_cslu_cz test_cslu_hi"
  ...
  if [ $decodedev -eq 1 ]; then
    for decode_set in $devsets; do
      online_ivector_dir=exp/nnet3/ivectors_${decode_set}_hires
